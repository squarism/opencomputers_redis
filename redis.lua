--------------------------------------------------------------------
-- Redis client for the Opencomputers minecraft mod.
-- Thank you to Daniele Alessandri for the real version.
-- This is alpha software!
-- @squarism 2015-02-28


-- You absolutely have to unblacklist localhost in opencomputers.cfg
-- if you are trying to connect to redis running on localhost
--     blacklist=[
--      "10.0.0.0/8",
--      "192.168.0.0/16",
--      "172.16.0.0/12"
...
-- opencomputers minecraft internet card
...
function network.write(client, buffer)
    local _, err = client.network.socket:write(buffer)
    if err then client.error(err) end
end

function network.read(client, len)
    if len == nil then len = '*l' end
    local line, err = client.network.socket:read(len)
...
local function create_connection(parameters)
    ...
    elseif parameters.scheme == "opencomputers" then
        -- sorry that I have to do a return in the middle, opencomputers api is
        -- different than a real socket
        local internet = require('internet')
        local handle = internet.open(parameters.host, parameters.port)
        return handle
    else
